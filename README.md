...
- `.env.traefik.example` documents this repository's local integrated Docker
  ...
  generated by `vendra-controller` — either the `vendra` host CLI or the
...
```bash
# Start Traefik and the backend with the vendra-controller CLI.
# (Build it once from ../vendra-controller: go build -o ./bin/vendra ./cmd/vendra)
...
- In production a property is rendered by `vendra-controller` from a Compose template embedded in its Go binary, using a digest-pinned image. This repository's Compose file is local integration tooling, not a second production definition — see `deploy/CONTRACT.md` for how the two differ.
...
For a manual or local deployment, use the `vendra` CLI from `vendra-controller`:
